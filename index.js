...
app.get("/developers", (req, res) => {
  // 1) URL'den arama sorgusunu al. eğer yoksa boş bir obje kullan.
  const searchQuery = req.query.search;
  let filter = {};

  // 2) eğer bir arama sorgusu varsa filtre objesini güncelle.
  if (searchQuery) {
    filter = { skills: { $regex: searchQuery, $options: "i" } };
  }

  // 3) veritabnı sorgusunu bu filtre ile yap
  Developer.find(filter)
    .sort({ createdAt: -1 })
    .then((result) => {
      res.render("developers", { developers: result });
    })
    .catch((err) => {
      console.log(err);
...
app.get("/developers/:id", (req, res) => {
  ...
  Developer.findById(id).then((result) => {
